...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def legalNeighbors: List[(Block, Move)] = {
      val legalN = scala.collection.mutable.MutableList.empty[(Block,Move)]
      if(left.isLegal) legalN ++= List((left,Left))
      if(right.isLegal) legalN ++= List((right,Right))
      if(up.isLegal) legalN ++= List((up,Up))
      if(down.isLegal) legalN ++= List((down,Down))
     ...
     */
    def isLegal: Boolean = {
      terrain.apply(b1) && terrain.apply(b2)
    }
  }
}
